# Main GIT operations
# git config
# git init 
# git clone - obtain repository from fixed URl
# git add - add to staging area
# git commit - records file to the version history
# git diff - show the changes that not yet staged
# git reset
# git status - lists files that had to be commited
# git rm
# git log - list the version history of current branch
# git show - shows the metadata and content changes of the cpecified commit
# git branch - lists the local branches in current repository
# git push - send the commited changes of master branch to your remote repository
# git pull - fetches and merge changes on the remote server to your working directory
# git stash - temporary stores all the modified tracked files
# git show - show the list of changes (hash)
# git revert - cancel of last commit
# git remote add 'shortname' URL - add remote repository
