...
# які зустрічаються в рядку більше ніж 1 раз. Рядок буде складатися лише з цифр та букв (великих і малих). Реалізуйте
# обчислення за допомогою генератора в один рядок
...
#    .......
def str_calc(user_str):
    return len(set((el for el in user_str.lower() if len(user_str) - len(user_str.lower().replace(el, '')) > 1)))


print(str_calc(input()))
